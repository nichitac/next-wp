...
// Navigation menu types and fetching
...
function parseNavigationHtml(
  ...
  while ((match = linkRegex.exec(html)) !== null) {
    ...
    if (wpUrl && href.startsWith(wpUrl)) {
      href = href.slice(wpUrl.length);
...
export async function getNavigation(): Promise<WPMenuItem[]> {
  const { navOverrides, navOrder } = await import("@/menu.config");

  const navigations = await wordpressFetchGraceful<WPNavigation[]>(
